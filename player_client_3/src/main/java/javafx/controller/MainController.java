...
public class MainController implements Initializable {
    ...
    public void exitFromApplication() {
        Platform.runLater(() -> {
            Main.server = null;
            playerBeanType_3.booleanPropertyKickFromServerProperty().setValue(true);
    ...
    public void startCountdownToTheEndOfRound(Integer roundTime) {
        ...
        KeyFrame frame = new KeyFrame(Duration.seconds(1), event -> {
            if (timeToEndOfRound <= 0) {
                try {
                    Main.server.sendPlayerAnswer(playerBeanType_3.getPlayerAnswers(3), Main.nickname, Main.captainNickname);

                } catch (RemoteException e) {
                    e.printStackTrace();
                }
                timeline.stop();
                playerBeanType_3.initNewRoundComponentsValues();
                playerBeanType_3.setStringPropertyCaptainCommand("");
            }

            playerBeanType_3.setIntegerPropertyTimeToEndOfRound(timeToEndOfRound);
            timeToEndOfRound--;
        });
    ...
    public PlayerClientBean getPlayerBeanType_3() {
        return playerBeanType_3;
    }

    public void endOfGame(List<PlayerClient> results) {
        Platform.runLater(() -> {
            Main.server = null;
            playerBeanType_3.booleanPropertyEndOfGameProperty().setValue(true);
            ...
            try {
                ...
                stage.setTitle("Listen To Your Captain - ver. Client no. 3");
